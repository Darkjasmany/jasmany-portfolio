...
const hasValidUrl = (url?: string): boolean => {
  return !!url && url !== "https://..." && url !== "#" && url !== "https://youtube.com/watch?v=...";
};
...
const ProjectModal: React.FC<ProjectModalProps> = ({ project, onClose }) => {
  ...
  const hasAnyLink = hasValidUrl(project.demoUrl) || hasValidUrl(project.githubUrl) || hasValidUrl(project.youtubeUrl);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      className="fixed inset-0 z-[999] bg-black/60 backdrop-blur-xl p-4 md:p-8 flex items-center justify-center"
    >
      {/* Contenedor principal */}
      <div className="relative w-full max-w-5xl bg-[var(--card)] rounded-3xl overflow-hidden border border-[var(--border)] shadow-2xl max-h-[92vh] animate-fade-in-up flex flex-col">
        {/* Botón cerrar */}
        <button
          onClick={onClose}
          aria-label="Cerrar modal"
          className="absolute top-5 right-5 z-50 w-10 h-10 rounded-full flex items-center justify-center bg-black/60 backdrop-blur-xl border border-white/20 text-white text-sm hover:bg-[var(--accent)] hover:scale-110 transition-all duration-200 cursor-pointer"
        >
          x
        </button>
        ...
        <div className="overflow-y-auto p-6 md:p-10 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          {/* ── HEADER: título + tecnologías ── */}
          <header className="mb-10 pr-12">
            <h2 className="text-3xl md:text-5xl font-black text-[var(--hero-text)] tracking-tight mb-6">
              {project.title}
            </h2>
            ...
            <div className="flex flex-wrap gap-2">
              {project.technologies.map(tech => (
                <span
                  key={tech}
                  className={`px-4 py-1.5 rounded-full text-sm font-semibold border ${getTechColor(tech) || "bg-[var(--tag)] text-[var(--tag-text)] border-[var(--border)]"}`}
                >
                  {tech}
                </span>
              ))}
          ...
          <div className="grid md:grid-cols-3 gap-10 mb-12">
            ...
            <aside className="flex flex-col gap-4">
              <h3 className="text-xl font-bold text-[var(--hero-text)] mb-1">
                Enlaces
              </h3>
              ...
              {hasValidUrl(project.demoUrl) && (
                <a
                  ...
                  className="w-full py-3 rounded-xl bg-[var(--accent)] text-white text-center font-semibold text-sm hover:brightness-110 transition-all duration-200"
                >
                  Visitar Demo
                </a>
              )}
              ...
              {hasValidUrl(project.githubUrl) && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full py-3 rounded-xl border-2 border-[var(--accent)] text-[var(--accent)] text-center font-semibold text-sm hover:bg-[var(--accent)] hover:text-white transition-all duration-200"
                >
                  Ver Código
                </a>
              )}

              {hasValidUrl(project.youtubeUrl) && (
                <a
                  href={project.youtubeUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full py-3 rounded-xl border-2 border-red-500 text-red-500 text-center font-semibold text-sm hover:bg-red-500 hover:text-white transition-all duration-200"
                >
                  Ver Video
                </a>
              )}
                ...
                {allImages.map((img, idx) => (
                  ...
                  >
                    <img
                      ...
                      className="w-full h-full object-contain md:object-cover transition-transform duration-6000 hover:scale-105"
                    />
                  </a>
                ))}
